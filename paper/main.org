...
#+attr_latex: :options [Introduce nodes]
...
Lemma [[lem:introduce-nodes]] shows how to compute the table for an introduce node \(x\) when we have the table for its child.
By Lemma [[lem:algorithm-x-feasible-permutation]], we can determine whether permutation \(\pi\) is feasible in \(O(\lvert\pi\rvert\cdot\lvert G\rvert)\) time.
...
#+attr_latex: :options [Forget nodes]
...
Rather than computing the table for a forget node \(x\) given the table for its child \(y\) in the obvious way suggested by Lemma [[lem:forget-nodes]], we use the fact that permutation \(\pi'\) is an \(X_y\)-extension of \(X_x\)-permutation \(\pi\) iff \(\pi\) is the \(X_x\)-restriction of \(\pi'\) and compute the table for \(x\) using Algorithm [[alg:forget-nodes]].

Let us quickly argue that, given a forget node \(x\) and the table for its child \(y\), the output \(D_x\) of Algorithm [[alg:forget-nodes]] satisfies Lemma [[lem:forget-nodes]].
...
Since we can compute any restriction of permutation \(\pi'\) in \(O(\lvert\pi'\rvert)\) time, Algorithm [[alg:forget-nodes]] computes the table for a forget node \(x\) given the table for its child \(y\) in \(O((k\lvert X_x\rvert)!+(k\lvert X_y\rvert)!k\lvert X_y\rvert)=O((k\lvert X_y\rvert)!k\lvert X_y\rvert)\) time.
...
Lemma [[lem:join-nodes]] shows how to compute the table for a join node \(x\) when we have the tables for both its children \(y_1\) and \(y_2\).
